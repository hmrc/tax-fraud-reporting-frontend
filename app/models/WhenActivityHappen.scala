...
sealed trait WhenActivityHappen

object WhenActivityHappen extends Enumerable.Implicits {
  ...
  val values: Seq[WhenActivityHappen] = Seq(
    OverFiveYears,
    BetweenOneAndFive,
  ...
  def options(implicit messages: Messages): Seq[RadioItem] = values.zipWithIndex.map {
    case (value, index) =>
      RadioItem(
        content = Text(messages(s"whenActivityHappen.${value.toString}")),
        value = Some(value.toString),
        id = Some(s"value_$index")
      )
  }

  implicit val enumerable: Enumerable[WhenActivityHappen] =
