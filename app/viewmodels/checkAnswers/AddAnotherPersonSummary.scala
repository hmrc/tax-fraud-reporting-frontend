    ...
    answers.get(AddAnotherPersonPage(Index(index))).map {
      answer =>
        ...
        val value = ValueViewModel(
          HtmlContent(
            HtmlFormat.escape(messages(s"addAnotherPerson.$answer"))
          )
        )
        ...
        SummaryListRowViewModel(
          ...
          value   = value,
          actions = Seq(
            ActionItemViewModel("site.change", routes.AddAnotherPersonController.onPageLoad(Index(index), CheckMode).url)
