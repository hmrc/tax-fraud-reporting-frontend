...
final case class IndividualBusinessDetailsPage(index: Index)
  ...
  override def cleanup(value: Option[IndividualBusinessDetails], userAnswers: UserAnswers): Try[UserAnswers] =
    if (value exists { _ != Yes })
      ...
      ).flatMap(_.remove(BusinessSelectCountryPage(index))).flatMap(_.remove(BusinessAddressPage(index))).flatMap(
        _.remove(BusinessContactDetailsPage(index))
      ).flatMap(_.remove(ReferenceNumbersPage(index))).flatMap(_.remove(SelectConnectionBusinessPage(index)))
