...
class SubmissionServiceSpec
  ...
  "submit" - {
    ...
    lazy val sanitizedIndividualAnswers: UserAnswers = UserAnswers("id")
      ...
      .set(IndividualAddressConfirmationPage(Index(0)), AddressResponse(
        lines = List("*", "*", "?"), town = Some("?"), postcode = Some("?"), country = Some("country")
      )).success.value
      ...
      .set(BusinessAddressConfirmationPage(Index(0)), AddressResponse(
        lines = List("?", "?", "?"),
        town = Some("?"), postcode = Some("?"), country = Some("country")
      )).success.value
      ...
      .set(YourContactDetailsPage, YourContactDetails(
        FirstName = null, LastName = "surname", Tel = "tel", Email = Some("email"), MemorableWord = Some("?")
      )).success.value
      .set(ActivitySourceOfInformationPage, ActivitySourceOfInformation.Other("source of information")).success.value
      .set(DocumentationDescriptionPage, "evidence details").success.value
    ...
    "must send the sanitized data and return a successful future when the server responds with CREATED for an individual payload" in {
      ...
      service.submit(sanitizedIndividualAnswers).futureValue
