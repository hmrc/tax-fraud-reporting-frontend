...
class BusinessAddressConfirmationControllerSpec extends SpecBase with MockitoSugar with TryValues with OptionValues {
  ...
  val controller = application.injector.instanceOf[BusinessAddressConfirmationController]

  "BusinessAddressConfirmation Controller" - {
    ...
    "must update user answers and redirect to the next page when a confirmed address is available" in {
      ...
      running(application) {
        ...
        val request = FakeRequest(
          GET,
          routes.BusinessAddressConfirmationController.onPageLoad(Index(0), NormalMode, Some("foo")).url
        )
    ...
    "must redirect to journey recovery controller if there is no confirmed address available" in {
      ...
      val request =
        FakeRequest(GET, routes.BusinessAddressConfirmationController.onPageLoad(Index(0), NormalMode, Some("foo")).url)
    ...
    "must fail if the id is missing from the request" in {
      val request =
        FakeRequest(GET, routes.BusinessAddressConfirmationController.onPageLoad(Index(0), NormalMode, None).url)
