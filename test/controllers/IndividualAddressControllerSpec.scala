...
class IndividualAddressControllerSpec extends AnyFlatSpec with MockitoSugar with ScalaFutures {
  ...
  private def scenario(userAnswers: JsObject)(test: (JsObject, IndividualAddressController) => Assertion) = {
    ...
    val individualAddressController = new IndividualAddressController(
      stubMessagesApi(),
    ...
    test(userAnswers, individualAddressController)
  }
  ...
  private def fakeDataRequest(userAnswers: JsObject = Json.obj(), bodyOpt: Option[Seq[(String, String)]] = None) = {
    ...
    DataRequest(fakeRequest.withSession(SessionKeys.sessionId -> "fakeSessionId"), mockId, UserAnswers("", userAnswers))
  }

  "IndividualAddressController" should "respond with status 200 given a request without a cached answer" in
    scenario(Json.obj("individualSelectCountry" -> "gb")) {
      (_, controller) =>
