...
import views.html.IndividualNationalInsuranceNumberView
...
class IndividualNationalInsuranceNumberControllerSpec extends SpecBase with MockitoSugar {
  ...
  "IndividualNationalInsuranceNumber Controller" - {
    ...
    "must return OK and the correct view for a GET" in {
      ...
      running(application) {
        val request = FakeRequest(GET, individualNationalInsuranceNumberRoute)

        val result = route(application, request).value

        val view = application.injector.instanceOf[IndividualNationalInsuranceNumberView]
    ...
    "must redirect to the next page when valid data is submitted" in {
      ...
      running(application) {
        val request =
          FakeRequest(POST, individualNationalInsuranceNumberRoute)
            .withFormUrlEncodedBody(("value", "AA123456C"))
