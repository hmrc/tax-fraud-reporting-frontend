...
import models.{Index, IndividualBusinessDetails, NormalMode, UserAnswers}
...
import views.html.IndividualBusinessDetailsView

import scala.concurrent.Future

class IndividualBusinessDetailsControllerSpec extends SpecBase {
  "IndividualBusinessDetails Controller" - {
    ...
    val form = (new IndividualBusinessDetailsFormProvider)()
    ...
    "must return OK and the correct view for a GET" in {
      ...
      running(application) {
        val request = FakeRequest(GET, individualBusinessDetailsRoute)

        val result = route(application, request).value

        val view = application.injector.instanceOf[IndividualBusinessDetailsView]
    ...
    "must redirect to the next page when valid data is submitted" in {
      ...
      running(application) {
        val request =
          FakeRequest(POST, individualBusinessDetailsRoute)
            .withFormUrlEncodedBody(("value", IndividualBusinessDetails.values.head.toString))
