...
import views.html.IndividualInformationView
...
class IndividualInformationControllerSpec extends SpecBase with MockitoSugar {
  ...
  "IndividualInformation Controller" - {
    ...
    "must return OK and the correct view for a GET" in {
      ...
      running(application) {
        val request = FakeRequest(GET, individualInformationRoute)

        val result = route(application, request).value

        val view = application.injector.instanceOf[IndividualInformationView]
    ...
    "must redirect to the next page when valid data is submitted" in {
      ...
      running(application) {
        val request =
          FakeRequest(POST, individualInformationRoute)
            .withFormUrlEncodedBody(("value[0]", IndividualInformation.values.head.toString))
