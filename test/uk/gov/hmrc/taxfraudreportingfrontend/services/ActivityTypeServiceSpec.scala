...
class ActivityTypeServiceSpec extends BaseSpec with Matchers with Mappings with GuiceOneAppPerSuite {

  val expectedActivities = Seq(
    ActivityType(
      "22030000",
  ...
  val activityTypeService: ActivityTypeService = app.injector.instanceOf[ActivityTypeService]

  "return the Activity Name by Code" should {

    "get a activityType by its code" in {
      activityTypeService.getActivityNameByCode("22030000") shouldBe Some("activityType.name.furlough")

    }
  }

  "return the Code by Activity Name" should {

    "get a activityType by its code" in {
      activityTypeService.getCodeByActivityName("activityType.name.furlough") shouldBe Some("22030000")
