...
class IndividualBusinessDetailsSpec extends AnyFreeSpec with Matchers with ScalaCheckPropertyChecks with OptionValues {

  "IndividualBusinessDetails" - {

    "must deserialise valid values" in {

      val gen = Gen.oneOf(IndividualBusinessDetails.values.toSeq)
    ...
    "must fail to deserialise invalid values" in {
      ...
      val gen = arbitrary[String] suchThat (!IndividualBusinessDetails.values.map(_.toString).contains(_))
