...
class BusinessChooseYourAddressSpec extends AnyFreeSpec with Matchers with ScalaCheckPropertyChecks with OptionValues {

  "BusinessChooseYourAddress" - {

    "must deserialise valid values" in {

      val gen = Gen.oneOf(BusinessChooseYourAddress.values.toSeq)
    ...
    "must fail to deserialise invalid values" in {
      ...
      val gen = arbitrary[String] suchThat (!BusinessChooseYourAddress.values.map(_.toString).contains(_))
