...
class ChooseYourAddressSpec extends AnyFreeSpec with Matchers with ScalaCheckPropertyChecks with OptionValues {

  "ChooseYourAddress" - {

    "must deserialise valid values" in {

      val gen = Gen.oneOf(ChooseYourAddress.values.toSeq)
    ...
    "must fail to deserialise invalid values" in {
      ...
      val gen = arbitrary[String] suchThat (!ChooseYourAddress.values.map(_.toString).contains(_))
